...
describe('Sidebar', () => {
  it('새 대화 버튼을 렌더링한다', () => {
    render(<Sidebar sessions={[]} onNewChat={vi.fn()}
      onSelectSession={vi.fn()} onDeleteSession={vi.fn()} />);
    expect(screen.getByText(/새 대화/)).toBeDefined();
  });

  it('세션 목록을 렌더링한다', () => {
    render(<Sidebar sessions={mockSessions} onNewChat={vi.fn()}
      onSelectSession={vi.fn()} onDeleteSession={vi.fn()} />);
  ...
  it('현재 세션을 강조 표시한다', () => {
    render(
      <Sidebar sessions={mockSessions} currentSessionId="1"
        onNewChat={vi.fn()} onSelectSession={vi.fn()} onDeleteSession={vi.fn()} />
    );
  ...
  it('새 대화 버튼 클릭 시 onNewChat을 호출한다', () => {
    ...
    render(<Sidebar sessions={[]} onNewChat={onNewChat}
      onSelectSession={vi.fn()} onDeleteSession={vi.fn()} />);
  ...
  it('세션 클릭 시 onSelectSession을 호출한다', () => {
    ...
    render(
      <Sidebar sessions={mockSessions} onNewChat={vi.fn()}
        onSelectSession={onSelectSession} onDeleteSession={vi.fn()} />
    );
  ...
  it('삭제 버튼 클릭 시 onDeleteSession을 호출한다', () => {
    ...
    render(
      <Sidebar sessions={mockSessions} onNewChat={vi.fn()}
        onSelectSession={vi.fn()} onDeleteSession={onDeleteSession} />
    );
